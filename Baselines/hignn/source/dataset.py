    ...
    def process(self):
        ...
        for smiles in smilesList:
            try:
                ...
                base_m = rdkit.Chem.MolFromSmiles(smiles, sanitize=False)
                base_m.UpdatePropertyCache(strict=False)
                new_mol = rdkit.Chem.SanitizeMol(base_m, rdkit.Chem.SanitizeFlags.SANITIZE_FINDRADICALS | 
                        rdkit.Chem.SanitizeFlags.SANITIZE_KEKULIZE | rdkit.Chem.SanitizeFlags.SANITIZE_SETAROMATICITY | 
                        rdkit.Chem.SanitizeFlags.SANITIZE_SETCONJUGATION | rdkit.Chem.SanitizeFlags.SANITIZE_SETHYBRIDIZATION | 
                        rdkit.Chem.SanitizeFlags.SANITIZE_SYMMRINGS, catchErrors=True)
                print(new_mol)
...
def load_dataset_random(path, dataset, seed, task_type, tasks=None, logger=None):
    ...
    trn = MolDataset(root=path, dataset='train_' + dataset, task_type=task_type, tasks=tasks, logger=logger)
    val = MolDataset(root=path, dataset='valid_' + dataset, task_type=task_type, tasks=tasks, logger=logger)
    test = MolDataset(root=path, dataset='test_' + dataset, task_type=task_type, tasks=tasks, logger=logger)
    '''
    ...
    if task_type == 'classification':
        ...
        for i in range(len(tasks)):
            pos_len = 0
            neg_len = 0
            for pyg_dataset in [trn, val, test]:
                validId = np.where((pyg_dataset.data.y[:, i] == 0) | (pyg_dataset.data.y[:, i] == 1))[0]
                pos_len_tmp = (pyg_dataset.data.y[:, i][validId].sum()).item()
                neg_len_tmp = len(pyg_dataset.data.y[:, i][validId]) - pos_len_tmp

                pos_len = pos_len + pos_len_tmp
                neg_len = neg_len + neg_len_tmp
...
def build_dataset(cfg, logger):
    cfg.defrost()
    task_name = get_task_names(os.path.join(cfg.DATA.DATA_PATH, 'raw/train_{}.csv'.format(cfg.DATA.DATASET)))
