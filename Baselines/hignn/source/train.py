...
@torch.no_grad()
def validate(cfg, model, criterion, dataloader, epoch, device, logger, eval_mode=False, save_result=False):
    model.eval()
    ...
    if save_result:
        for i in range(len(cfg.DATA.TASK_NAME)):
            df = pd.DataFrame(y_pred_list[i]).T
            df.to_csv(cfg.DATA.DATASET + f'_pred_{i}.csv', index=False)
            df_train = pd.DataFrame(y_label_list[i]).T
            df_train.to_csv(cfg.DATA.DATASET + f'_true_{i}.csv', index=False)
...
def train(cfg, logger):
    ...
    score = validate(cfg, model, criterion, test_loader, best_epoch, device, logger=logger, eval_mode=True, save_result=True)
