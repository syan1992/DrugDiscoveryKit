...
def atom_features(atom):
    res = one_of_k_encoding_unk(atom.GetSymbol(),['C','N','O','F','P','S','Cl','Br','I','B','Si','Unknown']) + \
                    one_of_k_encoding(atom.GetDegree(), [0, 1, 2, 3, 4, 5, 6]) + one_of_k_encoding_unk(atom.GetTotalNumHs(), [0, 1, 2, 3, 4]) + [atom.GetIsAromatic()] + one_of_k_encoding_unk(atom.GetFormalCharge(), [-1,0,1,3]) + \
...
if __name__ == '__main__':
    ...
    elif moldata in ['bbbp', 'sider', 'clintox', 'tox21', 'bace', 'hiv']:
        task = 'clas'
    ...
    for split in [1, 2, 3]:
        for split_type in ['train', 'test', 'valid']:
            processed_data_file = 'dataset/processed/' + split_type + '_' + moldata + task + '_' + str(split) + '_pyg.pt'
            print(processed_data_file)
            if not os.path.isfile(processed_data_file):
                try:
                    df = pd.read_csv('./dataset/raw/'+moldata+'_' + str(split) + '/raw/' + split_type + '_' + moldata + '_' + str(split) + '.csv')
                except:
                ...
                MolNet(root='./dataset', dataset=moldata+task, xd=smis, y=y, smile_graph=smile_graph, processed_file_name=split_type + '_' + moldata + task + '_' + str(split) + '_pyg.pt')
