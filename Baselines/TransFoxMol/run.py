...
def main(tasks, task, dataset, device, train_epoch, seed, fold, batch_size, rate, scaffold, modelpath, logger, lr, attn_head, output_dim, attn_layers, dropout, mean, stds, D, met, savem):
    ...
    if task == 'clas':
        ...
        loss_f = nn.BCEWithLogitsLoss(reduction='none').to(device)
        ...
        for fol in range(1, fold+1):
            ...
            print('dataset-------------------')
            print(dataset)
            train_loader, valid_loader, test_loader = load_data(dataset, batch_size, 0, fol)
            ...
            for i in range(1,train_epoch+1):
                ...
                if valid_auc > best_val_auc:
                    ...
                    test_loss, test_acc, test_f1, test_pre, test_rec, test_auc, preds, tars = testing(model, test_loader, loss_f, metric, task, device, mean, stds, True)
                    logger.info('Dataset: {}  Epoch: {:<3d}  test_loss: {:.4f}  test_acc: {:.4f}  test_f1: {:.4f}  test_auc: {:.4f}  test_pre: {:.4f}  test_rec: {:.4f}'.format(dataset, i, test_loss, test_acc, test_f1, test_auc, test_pre, test_rec))
            ...
            df = pd.DataFrame(best_preds.squeeze())
            df.to_csv('result//' + dataset + str(fol) + '_pred_result.csv')
            df = pd.DataFrame(tars.squeeze())
            df.to_csv('result//' + dataset + str(fol) + '_target_result.csv')
                ...
                if met == 'rmse':
                    if valid_rmse < best_val_rmse:
                        ...
                        if savem:
                            ...
                            torch.save(model.state_dict(), model_save_path)
                        test_loss, test_mae, test_rmse, test_r2, _, _, preds, tars = testing(model, test_loader, loss_f, metric, task, device, mean, stds, True)
                    ...
                    if valid_mae < best_val_rmse:
                        ...
                        if savem:
                            model_save_path = modelpath + '{}_{}_{}.pkl'.format(dataset, i, round(valid_rmse,4))
                            torch.save(model.state_dict(), model_save_path)
                        test_loss, test_mae, test_rmse, test_r2, _, _, preds, tars = testing(model, test_loader, loss_f, metric, task, device, mean, stds, False)
                        logger.info('Dataset: {}  Epoch: {:<3d}  test_loss: {:.4f}  test_mae: {:.4f}  test_rmse: {:.4f} test_r2: {:.4f}'.format(dataset, i, test_loss, test_mae, test_rmse, test_r2))
...
if __name__ == '__main__':
    ...
    if moldata in ['delaney', 'freesolv', 'lipo', 'qm7']:
        task = 'reg'
        numtasks = 1
        if moldata == 'delaney':
            labell = ['logSolubility']
